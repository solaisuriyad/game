...
class Game {
  constructor(canvas) {
    ...
    if (this.mode3d) {
      try {
        ...
        // 3D: movement is camera-relative (W = forward), mouse-look turns the view
  ...
  update(dt) {
    ...
    // toward each frame (stable, no jitter). A = turn right, D = turn left.
    if (this.mode3d && this.renderer3d) {
      const TURN = 2.8; // radians per second
      if (this.input.held('a')) this.renderer3d._targetYaw -= TURN * dt; // A = right
      if (this.input.held('d')) this.renderer3d._targetYaw += TURN * dt; // D = left
      this.renderer3d.updateCameraFollow(dt);
    }
    this.player.update(dt, this);
    // 3D mode: the player BODY faces the look direction (set by mouse movement
    // in the 3D renderer). When a monster attacks, the player auto-faces it
    ...
    if (this.mode3d && this.renderer3d) {
      ...
      if (engaged) {
        ...
        p.facing = this.renderer3d.lookYaw; // body faces where the camera looks
      }
    }
